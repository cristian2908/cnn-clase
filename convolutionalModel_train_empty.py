...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009, num_epochs = 100, minibatch_size = 10, print_cost = True):#
    """
    ...
    Y_test -- Etiquetas del conjunto de Test, de tamaño  (None, n_y = 6), mide el desempeño
    learning_rate -- factor de aprendizaje en la optimización-- factor de aprendizaje
    num_epochs -- Número de epocas en el ciclo de optimización--factor de convergencia
    minibatch_size -- Tamaño del minibatch--tama
    ...
    with tf.Session() as sess:
        ...
        # Run init
        sess.run(init)# al correr init cree variables de inicializacion 
        
        # Loop de entrenamiento
        for epoch in range(num_epochs): # proceso de entrenamiento, coge cada minibach y lo muestra a la red para mostrar la funcion de costo
            ...
            if epoch % 50 == 0:
                save_path = saver.save(sess, "ACTUALIZARCARPETA_NOMBREARCHIVO.ckpt")
        ...
        print(accuracy)
        train_accuracy = accuracy.eval({X: X_train[0:1000,...], Y: Y_train[0:1000,...]})
        test_accuracy = accuracy.eval({X: X_test[0:1000,...], Y: Y_test[0:1000,...]})
